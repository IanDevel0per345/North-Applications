...
class CancelCheckout(commands.Cog):
    ...
    async def on_close_cart_button(self, inter: disnake.MessageInteraction):
        ...
        if custom_id.startswith("close_cart:"):
            ...
            try:
                # Cancelar pagamento na Vision Pay API se aplicável e ainda não aprovado
                if cart.get("status") not in ["approved", "cancelled"]:
                    ...
                    if payment_provider == "vision_wallet":
                        try:
                            from functions.payments.vision_wallet import cancel_vision_payment_from_settings
                            ...
                            if payment_id:
                                # Cancelar na Vision Pay API
                                result = await cancel_vision_payment_from_settings(payment_id)
                                print(f"✅ Pagamento Vision Wallet {payment_id} cancelado ao encerrar: {result}")
                            else:
                                print(f"⚠️ Payment ID não encontrado para cancelamento Vision Wallet")
                                
                        except Exception as e:
                            # Não falhar o encerramento se houver erro ao cancelar na API
                            print(f"❌ Erro ao cancelar pagamento Vision Wallet: {e}")
    ...
    async def on_cancel_button(self, inter: disnake.MessageInteraction):
        ...
        if custom_id.startswith("cancel_checkout:"):
            ...
            # Cancelar pagamento na Vision Pay API se aplicável
            payment_data = cart.get("payment_data", {})
            ...
            if payment_provider == "vision_wallet":
                try:
                    ...
                    if payment_id:
                        # Cancelar na Vision Pay API
                        result = await cancel_vision_payment_from_settings(payment_id)
                        print(f"✅ Pagamento Vision Wallet {payment_id} cancelado: {result}")
                    else:
                        print(f"⚠️ Payment ID não encontrado para cancelamento Vision Wallet")
