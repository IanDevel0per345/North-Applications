...
def _extract_qr_image(data: Dict[str, Any]) -> tuple[Optional[bytes], Optional[str]]:
    ...
    # Tentar URL
    url = _find_first(data, ["qr_code_url", "qrcode_url", "qr_url", "image", "qr_code_image_url", "qrCodeImage"])  # Vision Wallet
...
def _extract_payment_ids(data: Dict[str, Any]) -> Dict[str, str]:
    ...
    out: Dict[str, str] = {}
    # Incluir txid para Efí, paymentId e correlationID para Vision Wallet
...
def _build_approved_checkout_message(
    ...
    if mode == "embed":
        # Modo Embed
        embed = disnake.Embed(
            title=f"{emoji.vision} Checkout Aprovado",
            color=embed_color,
        ...
        if discount_amount > 0 or balance_applied > 0:
            ...
            if cart.get("coupon_code"):
                payment_info += f"**Cupom:** `{cart.get('coupon_code')}`\n"
...
async def _build_cart_message(cart: Dict[str, Any], thread: disnake.Thread, mode: str) -> disnake.Message:
    ...
    def _get_available_payment_method_keys() -> list[str]:
        ...
        all_methods = {
            "pix": {
                "providers": ["vision_wallet", "mercado_pago", "efibank", "pagbank", "picpay", "pushinpay", "misticpay", "asaas", "pix_manual"]
            },
            "card": {
...
def _sanitize_error_for_user(error_msg: str) -> str:
    ...
    msg = re.sub(r'pay\.visionapplications\.com\.br[^\s]*', '', msg, flags=re.IGNORECASE)
...
async def _create_payment(
    ...
    if payment_method == "pix":
        # PIX: Vision Wallet, Mercado Pago, Efí, Pagbank, Manual, Assas Pix, PushinPay, PicPay
        
        # Tentar Vision Wallet PIX (prioridade)
        if "vision_wallet" not in providers_coming_soon and pagamentos.get("vision_wallet"):
            try:
                from functions.payments.vision_wallet import create_vision_payment_from_settings
                result = await create_vision_payment_from_settings(
                    value=amount,
                    description=description
                )
                if result:
                    result["_provider"] = "vision_wallet"
                return result
            except ValueError as e:
                # Erro de configuração faltando
                error_msg = _sanitize_error_for_user(str(e))
                errors.append(f"Vision Wallet: {error_msg}")
            except Exception as e:
                sanitized = _sanitize_error_for_user(str(e))
                errors.append(f"Vision Wallet: {sanitized}")
        
        # Tentar Mercado Pago PIX
        if "mercado_pago" not in providers_coming_soon:
            try:
        ...
        if "efibank" not in providers_coming_soon and (pagamentos.get("efibank") or pagamentos.get("efi")):
            try:
                result = await create_efi_payment_from_settings(
                    price=amount,
                    nome_pagador=user.display_name,
                    cpf_pagador="70419980105"
                )
                if result:
        ...
        # Tentar PIX Manual
        if "pix_manual" not in providers_coming_soon:
            try:
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if payment_provider:
                    provider_checkers = {
                        "vision_wallet": lambda pid: __import__('functions.payments.vision_wallet', fromlist=['check_vision_payment_from_settings']).check_vision_payment_from_settings(pid),
                    ...
                    if payment_method == "pix":
                        ...
                        from functions.payments.vision_wallet import check_vision_payment_from_settings as check_vision
                        pix_providers = [
                            ("vision_wallet", check_vision),
                            ("mercado_pago", check_mp_payment_from_settings),
                            ("efibank", check_efi_payment_from_settings),
                            ("pagbank", check_pagbank_payment_from_settings),
                            ("asaas", check_asaas_payment_from_settings),
            ...
            # Se não encontrou no nível superior, tentar em data.status (Vision Wallet pode retornar result diretamente)
...
async def _check_single_payment_status(
    ...
    try:
        ...
        # Verificar também em data.status (Vision Wallet pode retornar result diretamente)
        if status == "pending" and isinstance(chk, dict):
            data = chk.get("data", {})
            if isinstance(data, dict):
                data_status = data.get("status")
                    ...
                    if logs_cog:
                        ...
                        for item in items:
                            ...
                            await logs_cog.send_order_log(
                                ...
                                payment_method=cart.get("payment_method", "unknown"),
                                items=log_items,  # Itens entregues (None para entrega manual)
                                delivery_type=item_delivery_type,  # Usar tipo específico do item
                                cart_id=cart_id
                            )
                            
                            # Enviar evento público de compra para este item
