...
# Bridge para novo WSManager de connections
try:
    ...
    from bot.connections.ws_manager import WSManager
...
def get_websocket_manager():
    """Retorna a instância global do gerenciador WebSocket (bridge para novo WSManager)"""
    ...
    if websocket_manager is None:
        ...
        if bot_instance:
            websocket_manager = WSManager(bot_instance)
    return websocket_manager

def _get_http_base_url() -> str:
    """Obtém a URL base para chamadas HTTP da Vision Cloud API.
    Tenta `websocket.http_url`, depois `http.server_url`, caso contrário cai para `websocket.server_url`.
    """
    try:
        ...
        return (
            ...
            or "https://cloud.visionapplications.com.br"
        )
    except Exception:
        return "https://cloud.visionapplications.com.br"
...
async def _give_verified_role_internal(bot, auth_data: dict):
    ...
    try:
        ...
        if not already_has_role:
            await member.add_roles(verified_role, reason="Verificação automática via VisionCloud")
        ...
        if definitions.get("remove_autorole", {}).get("enabled", False):
            ...
            if not autorole_id:
                ...
                if not autorole:
                    ...
                    try:
                        await member.remove_roles(autorole, reason="Remoção de autorole após verificação via VisionCloud")
...
async def _remove_verified_role_internal(bot, auth_data: dict):
    ...
    try:
        ...
        # Remover o cargo
        await member.remove_roles(verified_role, reason="Desverificação automática via VisionCloud")
