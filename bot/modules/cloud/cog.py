...
class Cloud(commands.Cog):
    ...
    async def _gerenciar_gift(self, inter: disnake.MessageInteraction):
        ...
        try:
            ...
            if mode == "embed":
                ...
                embed = disnake.Embed(
                    title=f"Painel > VisionCloud > Gerenciar Gifts",
    ...
    async def _show_delete_options(self, inter: disnake.MessageInteraction):
        ...
        try:
            ...
            if mode == "embed":
                await embed_message.wait(inter)
                embed = disnake.Embed(
                    title=f"Painel > VisionCloud > Excluir Gifts",
                ...
                container = disnake.ui.Container(
                    disnake.ui.TextDisplay(f"# {emoji.vision}\n-# Painel > VisionCloud > **Excluir Gifts**"),
                    disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
    ...
    async def _load_gifts_for_manage(self, inter: disnake.MessageInteraction):
        ...
        try:
            ...
            # Fazer requisição para obter gifts
            ...
            if not ws_manager.is_connected():
                if mode == "embed":
                    ...
                    container = disnake.ui.Container(
                        disnake.ui.TextDisplay(f"# {emoji.wrong}\n-# **Erro**"),
                        disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
                        disnake.ui.TextDisplay("WebSocket não conectado."),
                    )
                    await inter.edit_original_response(components=[container])
                return
            ...
            if not gifts:
                ...
                if mode == "embed":
                    embed = disnake.Embed(
                        title=f"Painel > VisionCloud > Gerenciamento de Gifts",
                    ...
                    container = disnake.ui.Container(
                        disnake.ui.TextDisplay(f"# {emoji.vision}\n-# Painel > VisionCloud > **Gerenciamento de Gifts**"),
                        disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
    ...
    async def _load_gifts_for_delete(self, inter: disnake.MessageInteraction):
        ...
        try:
            ...
            if not gifts:
                ...
                if mode == "embed":
                    embed = disnake.Embed(
                        title=f"Painel > VisionCloud > Deletar Gifts",
                        description="Nenhum gift encontrado para este bot."
                    )
                    if primary_color_hex:
                        embed.color = disnake.Colour(int(primary_color_hex.replace("#", ""), 16))
                    ...
                    container = disnake.ui.Container(
                        disnake.ui.TextDisplay(f"# {emoji.vision}\n-# Painel > VisionCloud > **Deletar Gifts**"),
                        disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
                        disnake.ui.TextDisplay("Nenhum gift encontrado para este bot."),
                    )
                    await inter.edit_original_response(components=[container, disnake.ui.ActionRow(back_button)])
    ...
    async def on_button_click(self, inter: disnake.MessageInteraction):
        ...
        match inter.component.custom_id:
            case "Cloud_ManageTasks":
                ...
                if not bool(cloud_conf.get("client_id")):
                    if inter.response.is_done():
                        await inter.followup.send(f"{emoji.off} VisionCloud não está configurado. Use 'Editar Credenciais' primeiro.", ephemeral=True)
                    else:
                        await inter.response.send_message(f"{emoji.off} VisionCloud não está configurado. Use 'Editar Credenciais' primeiro.", ephemeral=True)
                    return
                mode = db.get_document("custom_mode").get("mode")
                if mode == "embed":
                    await embed_message.wait(inter)
            ...
            case "Cloud_CopyAuthURL":
                auth_url = "https://cloud.visionapplications.com.br/api/auth/callback"
                if inter.response.is_done():
            ...
            case "Cloud_GetAuthLink":
                ...
                redirect_uri = "https://cloud.visionapplications.com.br/api/auth/callback"
