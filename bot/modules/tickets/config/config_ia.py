...
class EditIAPromptModal(disnake.ui.Modal):
    def __init__(self, panel_id: str):
        ...
        super().__init__(title="Editar Instruções da VisionAI", components=components)
...
def ConfigIAView_components(inter: disnake.Interaction, panel_id: str) -> list:
    ...
    header_text = f"# {emoji.vision}\n-# Gerenciar Tickets > Editar Painel > {panel_name} > **VisionAI**"
    status_text = (
...
def ConfigIAView_embed(inter: disnake.Interaction, panel_id: str):
    ...
    embed = disnake.Embed(
        title=f"Configurar VisionAI: {panel_data.get('name')}",
