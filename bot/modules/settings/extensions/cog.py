...
class ExtensionsPanel(commands.Cog):
    ...
    def _get_visiongen_status(self) -> dict:
        """Obtém status da extensão Vision Gen"""
        return db.obter("database/visiongen.json")

    def _get_livestock_config(self) -> dict:
        """Obtém configuração de Live Stock"""
        data = db.obter("database/visiongen.json")
    ...
    def _build_components_panel(self, inter: disnake.MessageInteraction, extensions_config: dict) -> list:
        ...
        visiongen_data = self._get_visiongen_status()
        visiongen_enabled = visiongen_data.get("enabled", True)
        visiongen_integrated = visiongen_data.get("integrated_user_id") is not None

        # Construir status do Vision Gen
        if visiongen_enabled and visiongen_integrated:
            visiongen_status = f"{emoji.on} Ativo"
            visiongen_user = visiongen_data.get("integrated_user", "N/A")
        elif visiongen_enabled:
            visiongen_status = f"{emoji.warn} Não integrado"
            visiongen_user = None
        else:
            visiongen_status = f"{emoji.off} Desativado"
            visiongen_user = None
        ...
        extension_options = [
            disnake.SelectOption(
                ...
                value="visiongen",
                emoji=emoji.link,
                description=f"{'Ativo' if visiongen_enabled else 'Não integrado'} - Gerador de contas"
            ),
            disnake.SelectOption(
                label="Vision Boost",
                value="boost",
        ...
        # Vision Gen info
        visiongen_info = f"{emoji.link} **Vision Gen (Grátis)** — {visiongen_status}"
        if visiongen_user:
            visiongen_info += f"\n-# {emoji.member} Integrado com: `{visiongen_user}`"
        else:
            visiongen_info += f"\n-# Gerador de contas com integração ao painel web"

        # Vision Boost info
        boost_info = f"{emoji.boost} **Vision Boost** — {boost_status}"
        ...
        return [
            disnake.ui.Container(
                ...
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
                disnake.ui.TextDisplay(visiongen_info),
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
    ...
    def _build_embed_panel(self, inter: disnake.MessageInteraction, extensions_config: dict) -> tuple:
        ...
        # Vision Gen Field
        visiongen_value = f"**Status:** {visiongen_status}\n"
        if visiongen_integrated:
            visiongen_value += f"**Usuário:** `{visiongen_data.get('integrated_user', 'N/A')}`\n"
        visiongen_value += "Gerador de contas com integração ao painel web."

        embed.add_field(
            name=f"{emoji.link} Vision Gen (Grátis)",
            value=visiongen_value,
            inline=False
        )

        # Vision Boost Field
        embed.add_field(
            name=f"{emoji.boost} Vision Boost",
            value=(
    ...
    def _build_visiongen_panel(self, inter: disnake.MessageInteraction) -> list:
        """Constrói o painel do Vision Gen"""
        colors = db.get_document("custom_colors")
        primary_color_hex = colors.get("primary")
        ...
        status_data = self._get_visiongen_status()
        ...
        # Status info
        status_text = f"# {emoji.vision}\n-# Painel > Extensões > **Vision Gen**"
        ...
        return [
            disnake.ui.Container(
                ...
                disnake.ui.ActionRow(
                    disnake.ui.StringSelect(
                        custom_id="VisionGen_MainSelect",
            ...
            disnake.ui.ActionRow(
                ...
                disnake.ui.Button(
                    ...
                    custom_id="VisionGen_Toggle"
                )
            )
        ]

    def _build_visiongen_config_panel(self, inter: disnake.MessageInteraction) -> list:
        """Constrói o painel de configuração do Vision Gen"""
        colors = db.get_document("custom_colors")
        primary_color_hex = colors.get("primary")
        ...
        return [
            disnake.ui.Container(
                ...
                disnake.ui.ActionRow(
                    disnake.ui.StringSelect(
                        custom_id="VisionGen_ConfigSelect",
            ...
            disnake.ui.ActionRow(
                disnake.ui.Button(
                    ...
                    style=disnake.ButtonStyle.grey,
                    emoji=emoji.back,
                    custom_id="Extensions_VisionGen"
                )
            )
        ]
    ...
    async def on_extensions_button_click(self, inter: disnake.MessageInteraction):
        ...
        if custom_id == "Configuracoes_Extensoes":
            ...
            await inter.response.send_message(
                ...
                f"Todas as extensões são nomeadas pela **Vision**, porém são oferecidas através de "
        ...
        elif custom_id == "Extensions_VisionGen":
            mode = db.get_document("custom_mode").get("mode")
            if mode == "embed":
                await embed_message.wait(inter, send=False)
                # For embed mode, we still use components V2
                components = self._build_visiongen_panel(inter)
        ...
        elif custom_id == "VisionGen_Toggle":
            current_data = db.obter("database/visiongen.json")
            is_enabled = current_data.get("enabled", True)
            current_data["enabled"] = not is_enabled
            db.salvar("database/visiongen.json", current_data)
        ...
        elif custom_id == "VisionGen_BackToConfig":
            await message.wait(inter, send=False)
            components = self._build_visiongen_config_panel(inter)
            await inter.edit_original_message(components=components)

        elif custom_id == "VisionGen_ConfirmLiveStock":
        ...
        elif custom_id == "VisionGen_CancelLiveStock":
            await message.wait(inter, send=False)
            components = self._build_visiongen_config_panel(inter)
            await inter.edit_original_message(components=components)

        elif custom_id == "VisionGen_LiveStockAnalytics":
            await self._show_livestock_analytics(inter)

        elif custom_id == "VisionGen_LiveStockNotifications":
            await self._toggle_livestock_notifications(inter)
        
        elif custom_id == "VisionGen_LiveStockProducts":
            await self._show_products_for_livestock(inter)
    ...
    async def _show_my_payments(self, inter: disnake.MessageInteraction):
        ...
        # Header
        container_items.append(disnake.ui.TextDisplay(f"# {emoji.vision}\nMeus Pagamentos"))
    ...
    async def _show_purchase_panel(self, inter: disnake.MessageInteraction):
        ...
        purchase_select = disnake.ui.StringSelect(
            ...
            options=[
                disnake.SelectOption(
                    label="Vision Gen (Grátis)",
                    value="purchase_visiongen",
        ...
        # Texto do Vision Boost (com expiração se ativo)
        boost_title = f"{emoji.boost} **Vision Boost (R$50 per/month)**"
        if is_active and expiry_date:
            boost_title = f"{emoji.boost} **Vision Boost (Expira: {expiry_date})**"
        ...
        container = disnake.ui.Container(
            ...
            # Vision Gen Text
            disnake.ui.TextDisplay(
                f"{emoji.link} **Vision Gen (Grátis)**\n"
    ...
    async def _create_boost_payment(self, inter: disnake.MessageInteraction):
        ...
        embed = disnake.Embed(
            ...
            description=(
                f"Realize o pagamento via PIX para ativar o **Vision Boost**.\n\n"
    ...
    async def _verify_payment(self, inter: disnake.MessageInteraction, payment_id: str):
        ...
        if status == "COMPLETED":
            await inter.followup.send(
                ...
                f"A extensão **Vision Boost** foi ativada com sucesso.\n"
    ...
    async def on_visiongen_dropdown(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        ...
        if custom_id == "Extensions_Purchase_Select":
            ...
            if choice == "purchase_visiongen":
                 await inter.response.send_message(
                     f"{emoji.link} **Vision Gen** é uma extensão gratuita e já está disponível para uso.",
                     ephemeral=True
                 )
        ...
        if custom_id == "Extensions_Select":
            ...
            if choice == "visiongen":
                ...
                if boost_cog:
                    ...
                    await inter.response.send_message(
                        f"{emoji.warn} Extensão Vision Boost não está disponível.",
                        ephemeral=True
                    )
        ...
        if custom_id == "VisionGen_MainSelect":
            choice = inter.values[0]
            ...
            if choice == "generate_code":
                ...
                tutorial_text = (
                    ...
                    f"**Passo 2:** Acesse o **Dashboard** e clique em **Integrar Vision Bot**\n\n"
                    f"**Passo 3:** No bot, clique em **Gerar Código** no menu acima\n\n"
                    f"**Passo 4:** Copie o código gerado e cole no modal do site (que apareceu quando você clicou em Integrar Vision Bot)\n\n"
        ...
        elif custom_id == "VisionGen_ConfigSelect":
            choice = inter.values[0]
        ...
        elif custom_id == "VisionGen_ProjectSelect":
            val = inter.values[0]
        ...
        elif custom_id == "VisionGen_LiveStockProduct":
                    ...
                    if response.status == 200:
                        ...
                        components = [
                            disnake.ui.Container(
                                ...
                                disnake.ui.ActionRow(
                                    disnake.ui.StringSelect(
                                        custom_id="VisionGen_ProjectSelect",
            ...
            components = [
                ...
                disnake.ui.ActionRow(
                    disnake.ui.Button(
                        ...
                        style=disnake.ButtonStyle.green,
                        emoji=emoji.correct,
                        custom_id="VisionGen_ConfirmLiveStock"
                    ),
                    disnake.ui.Button(
                        label="Cancelar",
                        style=disnake.ButtonStyle.grey,
                        emoji=emoji.wrong,
                        custom_id="VisionGen_CancelLiveStock"
                    )
                )
            ]
    ...
    async def _show_products_for_livestock(self, inter: disnake.MessageInteraction):
        ...
        for product_id, p in products_dict.items():
            ...
            if product_name:
                ...
                visiongen_data = self._get_visiongen_status()
                livestock = visiongen_data.get("livestock", {})
        ...
        components = [
            disnake.ui.Container(
                ...
                disnake.ui.ActionRow(
                    disnake.ui.StringSelect(
                        custom_id="VisionGen_LiveStockProduct",
            ...
            disnake.ui.ActionRow(
                disnake.ui.Button(
                    ...
                    style=disnake.ButtonStyle.grey,
                    emoji=emoji.back,
                    custom_id="VisionGen_BackToConfig"
                ),
                disnake.ui.Button(
                    label="Análise",
                    style=disnake.ButtonStyle.blurple,
                    emoji=emoji.chart,
                    custom_id="VisionGen_LiveStockAnalytics"
                ),
                disnake.ui.Button(
                    label="Notificações",
                    style=disnake.ButtonStyle.grey,
                    emoji=emoji.announcement if hasattr(emoji, 'announcement') else emoji.message,
                    custom_id="VisionGen_LiveStockNotifications"
                )
            )
        ]
        ...
        components = [
            ...
            disnake.ui.ActionRow(
                disnake.ui.Button(
                    ...
                    style=disnake.ButtonStyle.grey,
                    emoji=emoji.back,
                    custom_id="VisionGen_LiveStockProducts"
                )
            )
        ]
    ...
    async def _toggle_livestock_notifications(self, inter: disnake.MessageInteraction):
        ...
        visiongen_data = db.obter("database/visiongen.json") or {}
        livestock = visiongen_data.setdefault("livestock", {})
        
        # Toggle notification setting
        current = livestock.get("dm_notifications", False)
        livestock["dm_notifications"] = not current
        
        db.salvar("database/visiongen.json", visiongen_data)
...
class LiveStockConfigModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        db.salvar("database/visiongen.json", data)
