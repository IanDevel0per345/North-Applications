...
class VisionGenConfig(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # However, the user's config.json has "https://api.visionapplications.com.br" which might be wrong or old.
    ...
    async def _check_pending_codes(self):
        ...
        status_data = db.obter("database/visiongen.json")
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        if data.get("redeemed"):
                            ...
                            db.salvar("database/visiongen.json", status_data)
        except Exception:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(
                    f"# {emoji.vision}\n"
                    f"-# Painel > Configurações > **Vision Gen**\n"
                ),
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
                disnake.ui.TextDisplay(
                    f"-# Painel de controle completo para o Vision Gen.\n"
    ...
    def panel_embed(self, inter: disnake.MessageInteraction) -> tuple:
        ...
        embed = disnake.Embed(
            title=f"{emoji.commands} Vision Gen",
            description=(
                f"-# Painel de controle completo para o Vision Gen."
                f"{emoji.on} **Status:** Ativo\n"
                f"{emoji.member} **Usuário Integrado:** `{integrated_user}`\n"
    ...
    async def on_visiongen_dropdown(self, inter: disnake.MessageInteraction):
        if inter.component.custom_id == "VisionGen_Select":
        ...
        elif inter.component.custom_id == "VisionGen_SystemSelect":
            choice = inter.values[0]
        ...
        elif inter.component.custom_id == "VisionGen_ProjectSelect":
            val = inter.values[0]
                        ...
                        components = [
                            ...
                            disnake.ui.ActionRow(
                                disnake.ui.Button(
                                    ...
                                    style=disnake.ButtonStyle.grey,
                                    emoji=emoji.back,
                                    custom_id="VisionGen_BackToMain" # Needs handler to go back to main VisionGen panel
                                )
                            )
                        ]
    ...
    async def _configure_system_menu(self, inter: disnake.MessageInteraction):
        ...
        components = [
            disnake.ui.Container(
                ...
                disnake.ui.ActionRow(
                    disnake.ui.StringSelect(
                        custom_id="VisionGen_SystemSelect",
            ...
            disnake.ui.ActionRow(
                disnake.ui.Button(
                    ...
                    style=disnake.ButtonStyle.grey,
                    emoji=emoji.back,
                    custom_id="VisionGen_BackToMain"
                )
            )
        ]
    ...
    async def _show_gen_category_selection(self, inter: disnake.MessageInteraction):
        ...
        components = [
            disnake.ui.ActionRow(
                disnake.ui.StringSelect(
                    custom_id="VisionGen_CategorySelect",
    ...
    async def on_visiongen_button_click(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        
        if custom_id == "VisionGen_BackToMain":
             await self.display_panel(inter) 
        
        elif custom_id == "VisionGen_BackToSystem":
             await self._configure_system_menu(inter)

        elif custom_id.startswith("VisionGen_OpenServiceModal_"):
            category = custom_id.replace("VisionGen_OpenServiceModal_", "")
        ...
        elif custom_id == "VisionGen_Tutorial":
            tutorial_text = (
                f"# {emoji.fire} Tutorial de Uso\n\n"
                f"**Como resgatar o código:**\n"
                f"1. Faça login no site [Loop Gen](https://loopgen.vercel.app).\n"
...
def setup(bot: commands.Bot):
    bot.add_cog(VisionGenConfig(bot))
