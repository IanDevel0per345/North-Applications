...
from functions.payments.vision_wallet import (
...
def _configured_providers() -> List[str]:
    ...
    # Vision Wallet
    if has((cfg.get("vision_wallet") or {}).get("api_key")):
        configured.append("vision_wallet")
    # PIX Manual
...
class PaymentCog(commands.Cog):
    ...
    async def pagamento(
        ...
        if not should_allow_payment_provider(key):
            await embed_message.error(
                inter, 
                f"{emoji.information} No plano atual, apenas **Vision Wallet** está disponível como forma de pagamento. Para usar outros métodos, faça upgrade do seu plano."
            )
            return
        ...
        if base_key not in configured:
            ...
            return
        try:
            if key == "vision_wallet":
                from functions.payments.vision_wallet import create_vision_payment_from_settings
                data = await create_vision_payment_from_settings(valor, description=descricao or f"Pagamento para {user.display_name}")
            ...
            error_msg = re.sub(r'pay\.visionapplications\.com\.br[^\s]*', '', error_msg, flags=re.IGNORECASE)
            ...
            for _ in range(180):
                ...
                try:
                    if key == "mercado_pago":
                        ...
                        chk = await check_vision_payment_from_settings(payment_id)
                    else:
                        chk = {}
                except Exception:
                    chk = {}
