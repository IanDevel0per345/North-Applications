...
from functions.emoji import emoji as vision_emoji
import hashlib
        ...
        if "roles" in tipos_set:
            for role in list(guild.roles):
                if not role.is_default():
                    try:
                        await role.delete(reason="[Vision] Backup restore - wipe roles")
                    except Exception:
                        pass
            created_roles = {}
            bot_member = guild.me
        ...
        if "members" in tipos_set and data.get('members'):
            ...
            for member_data in data.get('members', []):
                try:
                    ...
                    if roles_to_add:
                        await member.add_roles(*roles_to_add, reason="[Vision] Backup restore - members")
            ...
            try:
                ...
                if user:
                    ...
                    if mode == "embed":
                        ...
                        embed = disnake.Embed(
                            title=f"{vision_emoji.vision} Backup Restaurado com Sucesso!",
                        ...
                        embed.add_field(
                            ...
                            value=f"{vision_emoji.textc} **Canais:** `{get_count('channels')}`\n"
                                  f"{vision_emoji.dir} **Categorias:** `{get_count('categories')}`\n"
                                  f"{vision_emoji.role} **Cargos:** `{get_count('roles')}`\n"
                                  f"{vision_emoji.members} **Membros:** `{get_count('members')}`\n"
                                  f"{vision_emoji.reaction} **Emojis:** `{get_count('emojis')}`\n"
                                  f"{vision_emoji.flag} **Figurinhas:** `{get_count('stickers')}`\n"
                                  f"{vision_emoji.message if hasattr(vision_emoji, 'message') else '💬'} **Mensagens:** `{get_count('messages')}`\n"
                        )
                        ...
                        await dm.send(
                            components=[
                                disnake.ui.Container(
                                    ...
                                    disnake.ui.TextDisplay(
                                        f"{vision_emoji.textc} **Canais:** `{get_count('channels')}`\n"
                                        f"{vision_emoji.dir} **Categorias:** `{get_count('categories')}`\n"
                                        f"{vision_emoji.role} **Cargos:** `{get_count('roles')}`\n"
                                        f"{vision_emoji.members} **Membros:** `{get_count('members')}`\n"
                                        f"{vision_emoji.reaction} **Emojis:** `{get_count('emojis')}`\n"
                                    ...
                                    disnake.ui.Separator(),
                                    disnake.ui.TextDisplay(
                                        f"{vision_emoji.calendar} **Data:** <t:{agora}:f> (<t:{agora}:R>)\n"
                                    ),
                                )
                            ],
