    ...
    async def handle_wipe_confirm(self, inter: disnake.MessageInteraction, mode: str):
        ...
        if "channels" in tipos_set:
            for channel in list(guild.channels):
                try: await channel.delete(reason="[Vision] Backup wipe")
                except Exception as e: print(f"[Wipe] Erro ao apagar canal {getattr(channel, 'name', channel.id)}: {e}")
        if "roles" in tipos_set:
            for role in list(guild.roles):
                if not role.is_default():
                    try: await role.delete(reason="[Vision] Backup wipe")
                    except Exception as e: print(f"[Wipe] Erro ao apagar cargo {role.name}: {e}")
        if "emojis" in tipos_set:
            for emoji_item in list(guild.emojis):
                try: await emoji_item.delete(reason="[Vision] Backup wipe")
                except Exception as e: print(f"[Wipe] Erro ao apagar emoji {emoji_item.name}: {e}")
        if "stickers" in tipos_set and hasattr(guild, 'stickers'):
            for sticker in list(guild.stickers):
                try: await sticker.delete(reason="[Vision] Backup wipe")
                except Exception as e: print(f"[Wipe] Erro ao apagar figurinha {sticker.name}: {e}")
