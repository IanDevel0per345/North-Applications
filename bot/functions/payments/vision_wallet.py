"""
Vision Wallet Payment Integration
Integração com Vision Pay API para pagamentos PIX e gerenciamento de carteira virtual
"""
...
# URL base da Vision Pay API (pode ser configurada via env ou settings)
def _get_vision_pay_url() -> str:
    """Retorna a URL base da Vision Pay API"""
    # Tentar pegar do .env primeiro
    url = "https://api.visionwallet.com.br"
    return url.rstrip("/")
...
def _sanitize_error_message(error_msg: str) -> str:
    ...
    if not msg or len(msg) < 3:
        return "Erro ao processar com Vision Wallet. Verifique as configurações."
...
async def _request(
    ...
    """Faz requisição HTTP para a Vision Pay API"""
    base_url = _get_vision_pay_url()
    url = f"{base_url}/{path}"
    ...
    async with aiohttp.ClientSession(timeout=t) as session:
        async with session.request(method, url, json=payload, headers=headers) as resp:
            ...
            if data is None:
                raise RuntimeError("Resposta inválida do servidor Vision Pay")
...
def _require(value: Optional[str], what: str) -> str:
    ...
    if not value:
        raise ValueError(f"Missing {what} in Vision Wallet settings.")
    return value


def _get_api_key() -> str:
    """Retorna a API Key da Vision Wallet das configurações"""
    config = _load_config()
    vision_config = config.get("vision_wallet") or {}
    api_key = vision_config.get("api_key")
    return _require(api_key, "Vision Wallet API Key")


# ==================== PAGAMENTOS ====================

async def create_vision_payment(
    api_key: str,
    value: float,
    ...
    """
    Cria uma cobrança PIX via Vision Wallet
    
    Args:
        api_key: API Key do usuário na Vision Wallet
    ...
    if result.get("success") and result.get("data"):
        ...
        if qrcode_url:
            ...
            if isinstance(qrcode_url, str) and qrcode_url.startswith("data:image"):
                try:
                    ...
                    print(f"❌ Vision Wallet erro ao decodificar QR base64: {e}")
        ...
        if pix_code:
            try:
                ...
                print(f"❌ Vision Wallet erro ao gerar QR customizado: {e}")
...
async def check_vision_payment(api_key: str, payment_id: str) -> Dict[str, Any]:
    """
    Verifica o status de um pagamento na Vision Wallet
...
async def list_vision_payments(
    api_key: str,
...
async def cancel_vision_payment(api_key: str, payment_id: str) -> Dict[str, Any]:
    """
...
async def refund_vision_payment(
    api_key: str,
    payment_id: str,
...
async def create_vision_withdraw(
    api_key: str,
    amount: float,
...
async def get_vision_withdraw(api_key: str, withdraw_id: str) -> Dict[str, Any]:
    """
...
async def list_vision_withdraws(
    api_key: str,
    status: Optional[str] = None,
    limit: int = 50,
    offset: int = 0,
...
async def register_vision_user(
    name: str,
    email: str,
    tax_id: str,
    ...
    """
    Registra um novo usuário na Vision Wallet
...
async def get_vision_user(api_key: str) -> Dict[str, Any]:
    """
...
async def get_vision_balance(api_key: str) -> Dict[str, Any]:
    """
...
async def create_vision_payment_from_settings(
    value: float,
    description: Optional[str] = None,
    ...
    if cover_fee is None:
        config = _load_config()
        vision_config = config.get("vision_wallet") or {}
        cover_fee = vision_config.get("cover_fee", False)
    
    return await create_vision_payment(api_key, value, desc, cover_fee)


async def check_vision_payment_from_settings(payment_id: str) -> Dict[str, Any]:
    """Verifica pagamento usando API Key das configurações"""
    api_key = _get_api_key()
    return await check_vision_payment(api_key, payment_id)


async def list_vision_payments_from_settings(
    status: Optional[str] = None,
    limit: int = 50,
    offset: int = 0
) -> Dict[str, Any]:
    ...
    api_key = _get_api_key()
    return await list_vision_payments(api_key, status, limit, offset)


async def cancel_vision_payment_from_settings(payment_id: str) -> Dict[str, Any]:
    """Cancela pagamento usando API Key das configurações"""
    api_key = _get_api_key()
    return await cancel_vision_payment(api_key, payment_id)


async def refund_vision_payment_from_settings(
    payment_id: str,
    reason: Optional[str] = None
) -> Dict[str, Any]:
    """Reembolsa pagamento usando API Key das configurações"""
    api_key = _get_api_key()
    return await refund_vision_payment(api_key, payment_id, reason)


async def create_vision_withdraw_from_settings(
    amount: float,
    pix_key: Optional[str] = None,
    pix_key_type: Optional[str] = None
) -> Dict[str, Any]:
    ...
    api_key = _get_api_key()
    return await create_vision_withdraw(api_key, amount, pix_key, pix_key_type)


async def get_vision_withdraw_from_settings(withdraw_id: str) -> Dict[str, Any]:
    """Busca saque usando API Key das configurações"""
    api_key = _get_api_key()
    return await get_vision_withdraw(api_key, withdraw_id)


async def list_vision_withdraws_from_settings(
    status: Optional[str] = None,
    limit: int = 50,
    offset: int = 0,
    sync: bool = False
    ...
    api_key = _get_api_key()
    return await list_vision_withdraws(api_key, status, limit, offset, sync)


async def get_vision_user_from_settings() -> Dict[str, Any]:
    """Busca dados do usuário usando API Key das configurações"""
    api_key = _get_api_key()
    return await get_vision_user(api_key)


async def update_vision_user_from_settings(
    name: Optional[str] = None,
    email: Optional[str] = None,
    phone: Optional[str] = None,
    pix_key: Optional[str] = None,
    ...
    api_key = _get_api_key()
    return await update_vision_user(api_key, name, email, phone, pix_key, pix_key_type)


async def get_vision_balance_from_settings() -> Dict[str, Any]:
    """Busca saldo usando API Key das configurações"""
    api_key = _get_api_key()
    return await get_vision_balance(api_key)


__all__ = [
    # Pagamentos
    "create_vision_payment",
    "check_vision_payment",
    "list_vision_payments",
    "cancel_vision_payment",
    "refund_vision_payment",
    # Saques
    "create_vision_withdraw",
    "get_vision_withdraw",
    "list_vision_withdraws",
    # Usuário
    "register_vision_user",
    "get_vision_user",
    "update_vision_user",
    "get_vision_balance",
    # Wrappers com settings
    "create_vision_payment_from_settings",
    "check_vision_payment_from_settings",
    "list_vision_payments_from_settings",
    "cancel_vision_payment_from_settings",
    "refund_vision_payment_from_settings",
    "create_vision_withdraw_from_settings",
    "get_vision_withdraw_from_settings",
    "list_vision_withdraws_from_settings",
    "get_vision_user_from_settings",
    "update_vision_user_from_settings",
    "get_vision_balance_from_settings",
]
