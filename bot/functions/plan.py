...
def is_free() -> bool:
    """Verifica se o plano é Free (apenas Vision Wallet como método de pagamento)"""
...
def should_allow_payment_provider(provider_key: str) -> bool:
    ...
    Args:
        provider_key: Chave do provedor (ex: "vision_wallet", "mercado_pago", etc)
    ...
    if plan == "free":
        # No plano free, apenas Vision Wallet é permitido
        return provider_key == "vision_wallet"
...
def should_enable_cloud_button() -> bool:
    """Verifica se o botão VisionCloud deve estar habilitado no painel"""
    plan = get_plan()
    return plan in ["pro", "cloud"]
...
def should_enable_panel_button(button_name: str) -> bool:
    ...
    if plan == "cloud":
        # No plano cloud, apenas VisionCloud, Configurações e Personalização
